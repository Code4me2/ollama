...
var globalCommandResolver = NewCommandResolver()
...
// GetSystemRequirements returns a summary of system requirements and their status
func (cr *CommandResolver) GetSystemRequirements() map[string]interface{} {
	...
	// Check Node.js
	if cmd, err := cr.resolveNode(); err == nil {
		requirements["node"].(map[string]interface{})["status"] = "available"
		requirements["node"].(map[string]interface{})["command"] = cmd
	} else {
		requirements["node"].(map[string]interface{})["status"] = "not found"
	}

	// Check Python
	if cmd, err := cr.resolvePython(); err == nil {
		requirements["python"].(map[string]interface{})["status"] = "available"
		requirements["python"].(map[string]interface{})["command"] = cmd
	} else {
		requirements["python"].(map[string]interface{})["status"] = "not found"
	}

	// Check package manager
	if cmd, err := cr.resolveNodePackageManager(); err == nil {
		requirements["package_manager"].(map[string]interface{})["status"] = "available"
		requirements["package_manager"].(map[string]interface{})["command"] = cmd
	} else {
		requirements["package_manager"].(map[string]interface{})["status"] = "not found"
	}
