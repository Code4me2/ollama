...
func (m *MCPCodeAPI) GenerateMinimalContext(configs []api.MCPServerConfig) string {
	slog.Info("GenerateMinimalContext called", "configs_count", len(configs))
	if len(configs) == 0 {
		slog.Info("No MCP configs provided, returning empty context")
		return ""
	}
	...
	for _, config := range configs {
		slog.Info("Processing MCP config", "command", config.Command, "args", config.Args)
		...
		if isFilesystem && len(config.Args) > 1 {
			...
			slog.Info("Adding filesystem context", "working_dir", workingDir)
...
`, workingDir, workingDir, workingDir, workingDir, workingDir, workingDir))
	...
	slog.Info("Generated MCP context", "length", len(result), "context", result)
	return result
}
