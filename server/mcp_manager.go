...
// ToolResultCache provides caching of tool execution results
...
func NewMCPManager(maxClients int) *MCPManager {
	return &MCPManager{
		...
		cache:       NewToolResultCache(1000, 5*time.Minute), // Cache 1000 results for 5 minutes
...
func (m *MCPManager) ExecuteTool(toolCall api.ToolCall) ToolResult {
	...
	// Check cache first
	if cached := m.cache.Get(toolName, toolCall.Function.Arguments); cached != nil {
		slog.Debug("Tool result cache hit", "tool", toolName)
		return *cached
	}
	...
	if !exists {
		m.mu.RUnlock()
		result := ToolResult{Error: fmt.Errorf("tool '%s' not found", toolName)}
		m.cache.Set(toolName, toolCall.Function.Arguments, result)
		return result
	}
	...
	if !exists {
		m.mu.RUnlock()
		result := ToolResult{Error: fmt.Errorf("MCP client '%s' not found", clientName)}
		m.cache.Set(toolName, toolCall.Function.Arguments, result)
		return result
	}
	m.mu.RUnlock()
...
func (m *MCPManager) AnalyzeExecutionPlan(toolCalls []api.ToolCall) ExecutionPlan {
	...
	slog.Info("Execution plan analyzed", 
...
func (m *MCPManager) GetServerTools(serverName string) []api.Tool {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// Restart restarts all MCP clients (useful for error recovery)
func (m *MCPManager) Restart() error {
	m.mu.Lock()
	defer m.mu.Unlock()

	// Collect configurations before closing
	configs := make([]api.MCPServerConfig, 0, len(m.clients))
	for _, client := range m.clients {
	...
	for _, config := range configs {
		if err := m.addServerUnsafe(config); err != nil {
			slog.Error("Failed to restart MCP server", "name", config.Name, "error", err)
...
// addServerUnsafe adds a server without locking (for internal use)
func (m *MCPManager) addServerUnsafe(config api.MCPServerConfig) error {
	client := NewMCPClient(config.Name, config.Command, config.Args, config.Env)
...
func NewToolResultCache(maxSize int, ttl time.Duration) *ToolResultCache {
	cache := &ToolResultCache{
...
func (c *ToolResultCache) Get(toolName string, args api.ToolCallFunctionArguments) *ToolResult {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *ToolResultCache) Set(toolName string, args api.ToolCallFunctionArguments, result ToolResult) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *ToolResultCache) makeKey(toolName string, args api.ToolCallFunctionArguments) string {
	return fmt.Sprintf("%s:%s", toolName, args.String())
}
