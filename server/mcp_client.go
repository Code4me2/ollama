...
func NewMCPClient(name, command string, args []string, env map[string]string) *MCPClient {
	...
	// Resolve the command to handle different system configurations
	resolvedCommand := globalCommandResolver.ResolveForEnvironment(command)
...
func (c *MCPClient) Start() error {
	...
	// Create the command with security restrictions
	c.cmd = exec.CommandContext(c.ctx, cmdName, cmdArgs...)

	// Set up environment with security filtering
	c.cmd.Env = c.buildSecureEnvironment()

	// Configure process attributes for security isolation
	c.cmd.SysProcAttr = &syscall.SysProcAttr{
		// Process group isolation
		Setpgid: true,
		// Set process group ID to isolate the process
		Pgid: 0,
		// Drop privileges if running as root (commented out as it requires root)
...
func (c *MCPClient) CallTool(name string, args map[string]interface{}) (string, error) {
	...
	slog.Info("MCP tool response", "name", name, "is_error", resp.IsError, "content_count", len(resp.Content))

	if resp.IsError {
		slog.Error("MCP tool execution error", "name", name, "response", resp)
	...
	for {
		...
		default:
			...
			if err := json.Unmarshal(line, &resp); err != nil {
				slog.Warn("Invalid JSON-RPC response", "name", c.name, "line", string(line))
				continue
			}
...
func (c *MCPClient) handleErrors() {
	...
	for {
		...
		default:
			...
			slog.Debug("MCP server stderr", "name", c.name, "message", string(line))
		}
	}
}
...
func (c *MCPClient) buildSecureEnvironment() []string {
	...
	// Define allowed environment variables (safe defaults)
	allowedVars := map[string]bool{
		...
		"SHELL":      false, // Explicitly disallow
	}
	...
	for _, e := range os.Environ() {
		...
		if allowed, exists := allowedVars[key]; exists && allowed {
			// Sanitize PATH to remove potentially dangerous directories
			if key == "PATH" {
	...
	// Add custom environment variables after filtering
	for key, value := range c.env {
		// Ensure custom vars don't violate security policy
		if !securityConfig.ShouldFilterEnvironmentVar(key) {
			env = append(env, fmt.Sprintf("%s=%s", key, value))
		}
	}
...
// sanitizePath removes potentially dangerous directories from PATH
func sanitizePath(path string) string {
	// Split the PATH into components
	paths := strings.Split(path, ":")
	safePaths := []string{}
	
	// Define directories that should be excluded for security
	unsafeDirs := map[string]bool{
		"/root":           true,
		"/root/bin":       true,
		"./":              true,
		"../":             true,
		"~":               true,
		"/tmp":            false, // Allow /tmp but monitor usage
	}
	...
	for _, p := range paths {
		...
		// Check if path is in unsafe list
		isUnsafe := false
		for unsafeDir := range unsafeDirs {
			if strings.HasPrefix(absPath, unsafeDir) {
				isUnsafe = true
				break
			}
		}
		
		// Only include safe system directories
		if !isUnsafe && (strings.HasPrefix(absPath, "/usr/") || 
		   strings.HasPrefix(absPath, "/bin") || 
		   strings.HasPrefix(absPath, "/sbin") ||
		   strings.HasPrefix(absPath, "/opt/")) {
			safePaths = append(safePaths, p)
		}
	}
	
	// Always include basic system paths
	if len(safePaths) == 0 {
		safePaths = []string{"/usr/local/bin", "/usr/bin", "/bin"}
	}
	
	return strings.Join(safePaths, ":")
}
